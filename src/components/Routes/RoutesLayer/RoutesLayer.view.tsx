...
interface RoutesLayerProps {
  client: any;
  clickedLngLat: { lng: number; lat: number; } | undefined;
  ...
  addRouteToLocation: (LocationID:string, 
                      ...
                      departureTime: string) => void;
  selectedID: string;
  handleResetDepartureDestination: () => void;
};

// Get calculated route result
const callCalculateRouteCommand = async (
  client: any, 
  markers: MarkerType[], 
  options: any, 
) => {
...
const RoutesLayer = ({
  ...
  addRouteToLocation,
  selectedID,
  handleResetDepartureDestination
}: RoutesLayerProps) => {
  ...
  const handleTravelModeChange = (travelMode: string) => {
    setTravelMode(travelMode);
  }
  
  const handleDepartureTimeModeChange = (departureTimeMode: string) => {
    setDepartureTimeMode(departureTimeMode);
  }
  ...
  useEffect(() => {
    ...
    if (clickedLngLat && isOpenedPanel) {
      const markerData = new MarkerType(clickedLngLat, index);
  ...
  useEffect(() => {
    if (selectedDeparturePosition.address !== ""){
      const markerData = new MarkerType(selectedDeparturePosition.coordinates, index);
      setMarkers((current) => [...current, markerData]);
      setIndex(index + 1);
    }
  }, [selectedDeparturePosition]);
  ...
  useEffect(() => {
    if (selectedDeparturePosition.address !== "" && index > 0){
      const markerData = new MarkerType(selectedDestinationPosition.coordinates, index);
      setMarkers((current) => [...current, markerData]);
    }
  }, [selectedDestinationPosition]);

  const handleCalculate = async (options: any) => {
    try {
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div style={{ display: "flex", width: "24px", color: "black" }}>
      ...
      {isOpenedPanel && (
        <RoutesPanel
          ...
          handleDepartureTimeModeChange={handleDepartureTimeModeChange}
